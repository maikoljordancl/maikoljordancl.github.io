...
module.exports = async (req, res) => {
  ...
  const hfTokenLimpio = hfToken.trim();
  if (!hfTokenLimpio.startsWith("hf_")) {
    return res.status(500).json({ ok: false, error: `HUGGINGFACE_API_KEY parece inválida (debe empezar con "hf_"). Revisa que la copiaste completa y sin espacios en Vercel.` });
  }
  ...
  try {
    const client = new InferenceClient(hfTokenLimpio);
    ...
    let mensaje = (e && (e.message || e.toString())) || "Error al generar la imagen";
    if (e && e.cause) mensaje += " | causa: " + (e.cause.message || JSON.stringify(e.cause));
